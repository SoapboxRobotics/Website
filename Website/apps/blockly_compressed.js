	...
	for (var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0; 0 == c && g < f; g++) {
		...
		do {
			...
			c = goog.string.compareElements_(c, r) || goog.string.compareElements_(0 ==
				n[2].length, 0 == q[2].length) || goog.string.compareElements_(n[2], q[2])
} while (0 == c)
}
return c
};
		...
		goog.events.KeyCodes.firesKeyPressEvent = function(a, b, c, d, e) {
			...
			goog.events.EventHandler.prototype.listenOnce_ = function(a, b, c, d, e) {
				if (goog.isArray(b))
					...
					goog.events.EventTarget.dispatchEventInternal_ = function(a, b, c) {
						...
						Blockly.svgResize = function() {
							...
							Blockly.svg.cachedWidth_ != a && (Blockly.svg.setAttribute("width", a + "px"), Blockly.svg.cachedWidth_ = a);
							Blockly.svg.cachedHeight_ != b && (Blockly.svg.setAttribute("height", b + "px"), Blockly.svg.cachedHeight_ = b);
						...
						Blockly.onMouseDown_ = function(a) {
							...
							else if (!Blockly.editable || b) Blockly.mainWorkspace.dragMode = !0, Blockly.mainWorkspace.startDragMouseX = a.clientX, Blockly.mainWorkspace.startDragMouseY = a.clientY, Blockly.mainWorkspace.startDragMetrics = Blockly.getMainWorkspaceMetrics(),
								Blockly.mainWorkspace.startScrollX = Blockly.mainWorkspace.scrollX, Blockly.mainWorkspace.startScrollY = Blockly.mainWorkspace.scrollY
						};
				...
				Blockly.Block.prototype.dispose = function(a, b) {
					...
					for (c = this.childBlocks_.length - 1; 0 <= c; c--) this.childBlocks_[c].dispose(!1);
						this.mutator && this.mutator.dispose();
					this.comment && this.comment.dispose();
					this.warning && this.warning.dispose();
					for (c = 0; d = this.inputList[c]; c++) d.dispose();
						this.inputList = [];
					d = this.getConnections_(!0);
					for (c = 0; c < d.length; c++) e = d[c], e.targetConnection && e.disconnect(), d[c].dispose();
					...
					if ("mousemove" != a.type || 1 != a.x || 0 != a.y || 0 != a.button) {
						...
						if (2 == Blockly.Block.dragMode_) {
							...
							for (d = 0; d < this.draggedBubbles_.length; d++) e = this.draggedBubbles_[d], e.bubble.setIconLocation(e.x + b, e.y + c);
								...
								f && f != Blockly.highlightedConnection_ && (f.highlight(),
									Blockly.highlightedConnection_ = f, Blockly.localConnection_ = g);
			...
			Blockly.Block.prototype.setParent = function(a) {
				...
				Blockly.Block.prototype.setCollapsed = function(a) {
					if (this.collapsed != a) {
						for (var b = (this.collapsed = a) ? "none" : "block", c = [], d = 0, e; e = this.inputList[d]; d++) {
							for (var f = 0, g; g = e.titleRow[f]; f++)(g.getRootElement ? g.getRootElement() : g).style.display = b;
				...
				Blockly.BlockSvg.prototype.renderCompute_ = function(a) {
					...
					for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, l = 0, m; m = b[l]; l++) {
						var n;
						this.block_.inputsInline && k && k != Blockly.NEXT_STATEMENT && m.type != Blockly.NEXT_STATEMENT ? n = c[c.length - 1] : (k = m.type, n = [],
							n.type = this.block_.inputsInline && m.type != Blockly.NEXT_STATEMENT ? Blockly.BlockSvg.INLINE : m.type, n.height = 0, c.push(n));
						n.push(m);
						...
						for (var q = 0, t; t = m.titleRow[q]; q++) 0 != q && (m.titleWidth += Blockly.BlockSvg.SEP_SPACE_X), t = t.getSize(), m.titleWidth += t.width, n.height = Math.max(n.height, t.height);
							n.type != Blockly.BlockSvg.INLINE && (n.type == Blockly.NEXT_STATEMENT ? (g = !0, e = Math.max(e, m.titleWidth)) : (n.type == Blockly.INPUT_VALUE ? f = !0 : n.type == Blockly.DUMMY_INPUT && (h = !0), d = Math.max(d,
								m.titleWidth)))
					}
					for (a = 0; n = c[a]; a++)
								...
								for (var h, k = 0, l, m, n = 0, q; q = f[n]; n++) {
									...
									if (q.type == Blockly.BlockSvg.INLINE) {
										for (var r = 0, p; p = q[r]; r++) l = k + Blockly.BlockSvg.TITLE_HEIGHT, q.thicker && (l += Blockly.BlockSvg.SEP_SPACE_Y), h = this.renderTitles_(p.titleRow, h, l), p.type != Blockly.DUMMY_INPUT && (h += p.renderWidth + Blockly.BlockSvg.SEP_SPACE_X), p.type == Blockly.INPUT_VALUE && (c.push("M", h - Blockly.BlockSvg.SEP_SPACE_X +
											...
											Blockly.BlockSvg.SEP_SPACE_X - p.renderWidth + 1, m = e.y + k + Blockly.BlockSvg.SEP_SPACE_Y, p.connection.moveTo(l, m), p.connection.targetConnection && p.connection.tighten_());
h = Math.max(h, f.rightEdge);
a.push("H", h);
b.push("H", h + (Blockly.RTL ? -1 : 0));
a.push("v", q.height);
Blockly.RTL && b.push("v", q.height - 2)
} else q.type == Blockly.INPUT_VALUE ? (p = q[0], l = k + Blockly.BlockSvg.TITLE_HEIGHT, p.align != Blockly.ALIGN_LEFT && (r = f.rightEdge - p.titleWidth - Blockly.BlockSvg.TAB_WIDTH - Blockly.BlockSvg.SEP_SPACE_X, p.align == Blockly.ALIGN_RIGHT ?
	h = r : p.align == Blockly.ALIGN_CENTRE && (h = (r + h) / 2)), this.renderTitles_(p.titleRow, h, l), a.push(Blockly.BlockSvg.TAB_PATH_DOWN), a.push("v", q.height - Blockly.BlockSvg.TAB_HEIGHT), Blockly.RTL ? (b.push(Blockly.BlockSvg.TAB_PATH_DOWN_HIGHLIGHT_RTL), b.push("v", q.height - Blockly.BlockSvg.TAB_HEIGHT)) : (b.push("M", f.rightEdge - 4.2 + "," + (k + Blockly.BlockSvg.TAB_HEIGHT - .4)), b.push("l", .42 * Blockly.BlockSvg.TAB_WIDTH + ",-1.8")), l = e.x + (Blockly.RTL ? -f.rightEdge - 1 : f.rightEdge + 1), m = e.y + k, p.connection.moveTo(l, m), p.connection.targetConnection &&
p.connection.tighten_()) : q.type == Blockly.DUMMY_INPUT ? (p = q[0], l = k + Blockly.BlockSvg.TITLE_HEIGHT, p.align != Blockly.ALIGN_LEFT && (r = f.rightEdge - p.titleWidth - Blockly.BlockSvg.SEP_SPACE_X, f.hasValue && (r -= Blockly.BlockSvg.TAB_WIDTH), p.align == Blockly.ALIGN_RIGHT ? h = r : p.align == Blockly.ALIGN_CENTRE && (h = (r + h) / 2)), this.renderTitles_(p.titleRow, h, l), a.push("v", q.height), Blockly.RTL && b.push("v", q.height - 2)) : q.type == Blockly.NEXT_STATEMENT && (p = q[0], 0 == n && (a.push("v", Blockly.BlockSvg.SEP_SPACE_Y), Blockly.RTL && b.push("v",
	Blockly.BlockSvg.SEP_SPACE_Y - 1), k += Blockly.BlockSvg.SEP_SPACE_Y), l = k + Blockly.BlockSvg.TITLE_HEIGHT, p.align != Blockly.ALIGN_LEFT && (r = f.statementEdge - p.titleWidth - Blockly.BlockSvg.SEP_SPACE_X, p.align == Blockly.ALIGN_RIGHT ? h = r : p.align == Blockly.ALIGN_CENTRE && (h = (r + h) / 2)), this.renderTitles_(p.titleRow, h, l), h = f.statementEdge + Blockly.BlockSvg.NOTCH_WIDTH, a.push("H", h), a.push(Blockly.BlockSvg.INNER_TOP_LEFT_CORNER), a.push("v", q.height - 2 * Blockly.BlockSvg.CORNER_RADIUS), a.push(Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER),
a.push("H", f.rightEdge), Blockly.RTL ? (b.push("M", h - Blockly.BlockSvg.NOTCH_WIDTH + Blockly.BlockSvg.DISTANCE_45_OUTSIDE + "," + (k + Blockly.BlockSvg.DISTANCE_45_OUTSIDE)), b.push(Blockly.BlockSvg.INNER_TOP_LEFT_CORNER_HIGHLIGHT_RTL), b.push("v", q.height - 2 * Blockly.BlockSvg.CORNER_RADIUS), b.push(Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER_HIGHLIGHT_RTL), b.push("H", f.rightEdge - 1)) : (b.push("M", h - Blockly.BlockSvg.NOTCH_WIDTH + Blockly.BlockSvg.DISTANCE_45_OUTSIDE + "," + (k + q.height - Blockly.BlockSvg.DISTANCE_45_OUTSIDE)),
	b.push(Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER_HIGHLIGHT_LTR), b.push("H", f.rightEdge)), l = e.x + (Blockly.RTL ? -h : h), m = e.y + k + 1, p.connection.moveTo(l, m), p.connection.targetConnection && p.connection.tighten_(), n == f.length - 1 || f[n + 1].type == Blockly.NEXT_STATEMENT) && (a.push("v", Blockly.BlockSvg.SEP_SPACE_Y), Blockly.RTL && b.push("v", Blockly.BlockSvg.SEP_SPACE_Y - 1), k += Blockly.BlockSvg.SEP_SPACE_Y);
k += q.height
}
f.length || (this.block_.collapsed && (a.push(Blockly.BlockSvg.JAGGED_TEETH), Blockly.RTL ? b.push("l 8,0 0,3.8 7,3.2 m -14.5,9 l 8,4") :
	b.push("h 8")), k = Blockly.BlockSvg.MIN_BLOCK_Y, a.push("V", k), Blockly.RTL && b.push("V", k - 1));
return k
};
...
Blockly.Connection.prototype.closest = function(a, b, c) {
	function d(b) {
		var c = e[b];
		if ((c.type == Blockly.OUTPUT_VALUE || c.type == Blockly.PREVIOUS_STATEMENT) && c.targetConnection || !m.checkType_(c)) return !0;
		c = c.sourceBlock_;
		do {
			if (l == c) return !0;
			c = c.getParent()
...
Blockly.Connection.prototype.hideAll = function() {
	...
	Blockly.Connection.prototype.unhideAll = function() {
		...
		if (b) {
			...
			for (var d = 0; d < c.length; d++) a = a.concat(c[d].unhideAll());
				0 == a.length && (a[0] = b);
			b.comment &&
			b.comment.isPinned() && b.comment.setVisible_(!0)
		}
		return a
	};
	...
	Blockly.ConnectionDB.prototype.removeConnection_ = function(a) {
		...
		Blockly.ContextMenu.show = function(a, b, c) {
			if (0 == c.length) Blockly.ContextMenu.hide();
			else {
				...
				for (f = 0; f < e.length; f++) e[f].setAttribute("width", d);
					if (Blockly.RTL)
						for (f = 0; h = Blockly.ContextMenu.svgOptions.childNodes[f]; f++) l = h.lastChild, l.setAttribute("text-anchor", "end"), l.setAttribute("x", d - Blockly.ContextMenu.X_PADDING);
				...
				Blockly.FieldDropdown.prototype.showEditor_ = function() {
					...
					f +=
					2 * Blockly.ContextMenu.X_PADDING;
					for (l = 0; l < g.length; l++) g[l].setAttribute("width", f);
						if (Blockly.RTL)
							for (l = 0; n = c.childNodes[l]; l++) r = n.lastChild, r.setAttribute("text-anchor", "end"), r.setAttribute("x", f - Blockly.ContextMenu.X_PADDING);
						...
						Blockly.FieldDropdown.prototype.getValue = function() {
							for (var a = this.text_, b = this.getOptions_(), c = 0; c < b.length; c++)
						...
						Blockly.FieldDropdown.prototype.setValue = function(a) {
							...
							function findById(a, b) {
								for (var c = 0; c < a.length; c++)
									if (a[c] === b || "object" === typeof a[c] && a[c].name === b) return c;
								return -1
							}
							Blockly.FieldVariable.dropdownCreate = function() {
								...
								for (var b = [], c = 0; c < a.length; c++) b[c] = "string" === typeof a[c] ? [a[c], a[c]] : [a[c].name, a[c]];
									return b
							};
							Blockly.FieldVariable.dropdownChange = function(a) {
								...
								} else a == Blockly.MSG_NEW_VARIABLE && (a = b(Blockly.MSG_NEW_VARIABLE_TITLE, ""), Blockly.Variables.renameVariable(a, a)), a && this.setText(a);
							...
							Blockly.Flyout.prototype.position_ = function() {
								...
								if (a) {
									...
									c.push("a", this.CORNER_RADIUS, this.CORNER_RADIUS, 0, 0, Blockly.RTL ? 0 : 1, Blockly.RTL ? this.CORNER_RADIUS :
							...
							Blockly.Flyout.prototype.hide = function() {
								...
								Blockly.Flyout.prototype.show = function(a) {
									...
									var c = [],
									d = [];
									if (a == Blockly.MSG_VARIABLE_CATEGORY) Blockly.Variables.flyoutCategory(c, d, b, this.workspace_);
									else if (a == Blockly.MSG_PROCEDURE_CATEGORY) Blockly.Procedures.flyoutCategory(c, d, b, this.workspace_);
									else
										for (var e = 0, f; f = a[e]; e++) f = new Blockly.Block(this.workspace_, f), f.initSvg(), c[e] = f, d[e] = 2 * b;
											a = 0;
										...
										for (e = 0; f = c[e]; e++) Blockly.RTL && f.moveBy(a - b - Blockly.BlockSvg.TAB_WIDTH, 0), h = f.getSvgRoot().getBBox(), d = f.getRelativeToSurfaceXY(), d = Blockly.createSvgElement("rect", {
											width: h.width,
											height: h.height,
											x: d.x + h.x,
											y: d.y +
										...
										Blockly.CodeGenerator.prototype.valueToCode = function(a, b, c) {
											...
											if ("" === a) return "";
											if (!(a instanceof Array)) throw 'Expecting tuple from value block "' + b.type + '".';
											b = a[0];
											a = a[1];
											b && c <= a && (b = "(" + b + ")");
											return b
										...
										Blockly.createDom_ = function(a) {
											...
											if (!c) throw "No head in document.";
											c.appendChild(b);
										...
										Blockly.Procedures.flyoutCategory = function(a, b, c, d) {
											function e(e, f) {
												for (var k = 0; k < e.length; k++) {
													...
													for (var m = [], n = 0; n < e[k][1].length; n++) m[n] = "ARG" + n;
								...
								Blockly.ScrollbarPair.prototype.resize = function() {
									...
									if (a) {
										var b = !1,
										c = !1;
										if (this.oldHostMetrics_.viewWidth != a.viewWidth || this.oldHostMetrics_.viewHeight != a.viewHeight || this.oldHostMetrics_.absoluteTop != a.absoluteTop || this.oldHostMetrics_.absoluteLeft != a.absoluteLeft) c = b = !0;
										else {
											if (this.oldHostMetrics_.contentWidth != a.contentWidth || this.oldHostMetrics_.viewLeft != a.viewLeft || this.oldHostMetrics_.contentLeft != a.contentLeft) b = !0;
											if (this.oldHostMetrics_.contentHeight != a.contentHeight ||
												this.oldHostMetrics_.viewTop != a.viewTop || this.oldHostMetrics_.contentTop != a.contentTop) c = !0
										}
						...
						Blockly.Toolbox.redraw = function() {
							...
							c = 0;
							for (var e = [Blockly.Toolbox.svgBackground_], f = 0; d = b[f]; f++) {
								...
								g.cat = d.cat;
								Blockly.bindEvent_(g,
									"mousedown", null, a(g));
								e.push(h);
								c = Math.max(c, k.getComputedTextLength())
							}
							c += 2 * Blockly.ContextMenu.X_PADDING;
							for (f = 0; f < e.length; f++) e[f].setAttribute("width", c);
								if (Blockly.RTL)
									for (f = 0; g = Blockly.Toolbox.svgOptions_.childNodes[f]; f++) k = g.lastChild, k.setAttribute("text-anchor", "end"), k.setAttribute("x", c - Blockly.ContextMenu.X_PADDING);
								...
								Blockly.Toolbox.selectOption_ = function(a) {
									...
									if (Blockly.Toolbox.selectedOption_ = a) Blockly.addClass_(a, "blocklyMenuSelected"), a = a.cat, Blockly.Toolbox.flyout_.show(Blockly.Toolbox.languageTree[a] || a)
								};
							...
							Blockly.Tooltip.onMouseMove_ = function(a) {
								...
								Blockly.Tooltip.show_ = function() {
									...
									Blockly.Trashcan.prototype.setOpen_ = function(a) {
										this.isOpen != a && (window.clearTimeout(this.lidTask_), this.isOpen = a, Blockly.Trashcan.animateLid_(this))
									};
									Blockly.Trashcan.animateLid_ = function(a) {
										...
										if (a.isOpen ? 45 > a.lidAngle_ : 0 < a.lidAngle_) a.lidTask_ = window.setTimeout(function() {
										...
										Blockly.Variables.flyoutCategory = function(a, b, c, d) {
											...
											for (g = 0; g < e.length; g++)
												if (null == e[g] || e[g].name !== f) {
													...
													h && h.initSvg();
													var k = Blockly.Language.variables_set ? new Blockly.Block(d, "variables_set") :
													null;
													k && k.initSvg();
											...
											Blockly.Variables.generateUniqueName = function() {
												...
												if (a.length) {
													...
													for (var c = 0, d = "i", e = 0, f = !1; !b;) {
														e = 0;
														for (f = !1; e < a.length && !f;) a[e].name.toLowerCase() == d && (f = !0), e++;
											...
											Blockly.Warning.prototype.setVisible = function(a) {
												if (a != this.isVisible())
													...
													Blockly.Workspace.prototype.addTrashcan = function(a) {
														Blockly.Trashcan && this.editable && (this.trashcan = new Blockly.Trashcan(a), a = this.trashcan.createDom(), this.svgGroup_.insertBefore(a, this.svgBlockCanvas_), this.trashcan.init())
													};
															...
															Blockly.Xml.domToBlock_ = function(a, b) {
																...
																for (var d = null, e = 0, f; f = b.childNodes[e]; e++)
																	if (3 != f.nodeType || !f.data.match(/^\s*$/)) {
																		for (var g = null, h = 0, k; k = f.childNodes[h]; h++) 3 == k.nodeType && k.data.match(/^\s*$/) || (g = k);
