compilerflasher = function(lf) {
    ...
    this.pluginHandler = new function() {
        ...
        this.doflashBootloader = function(programmer, board) {
            document.getElementById('plugin0').flashBootloader(
                ...
                board['bootloader']['low_fuses'], (typeof board['bootloader']['extended_fuses'] === "undefined") ? '' : board['bootloader']['extended_fuses'], (typeof board['bootloader']['unlock_bits'] === "undefined") ? '' : board['bootloader']['unlock_bits'], (typeof board['bootloader']['lock_bits'] === "undefined") ? '' : board['bootloader']['lock_bits'],
                board['build']['mcu'],
                bootloader_callback
            );
        }
        ...
        this.clickedPort = function() {
            ...
            url = url.replace("CLICK_PORT_META", JSON.stringify({
                "selectedPort": port,
                "tabID": this.tabID
            }));
            $.get(url);
        ...
        this.logPorts = function() {
            ...
            if (oldPort != newPort) {
                ...
                url = url.replace("SAVEPORTMETA", JSON.stringify({
                    "oldPort": oldPort,
                    "newPort": newPort,
                    "tabID": this.tabID
                }));
            ...
            window.PluginLoggingInterval = setInterval(function() {
                try {
                    if (typeof(document.getElementById('plugin0').probeUSB) === "undefined") {
                        ...
                        if ((typeof document.getElementById('plugin0').availablePorts === 'undefined') && ((oldPortsAvail.length < portsAvail.length || (oldPortsAvail.length == 1 && portsAvail.length == 1 && oldPortsAvail[0] == "" && portsAvail[0] != "")) || (oldPortsAvail.length > portsAvail.length || (oldPortsAvail.length == 1 && portsAvail.length == 1 && oldPortsAvail[0] != "" && portsAvail[0] == "")))) {
                            var ports = Object();
        ...
        this.addTo = function(where, pluginUrl) {
            ...
            url = url.replace("ADD_TO_META", JSON.stringify({
                "where": where
            }));
        ...
        this.getFire = function() {
            try {
                ...
                /*clearInterval(this.getFireInterval);
	var alert = this.browserSpecificPluginInstall("To program your Arduino from your browser, install the Codebender Plugin. ");
	url = "https\x3A\x2F\x2Fcodebender.cc\x2Futilities\x2Flogdb\x2F35\x2FPLUGIN_META";
	...
	var _pluginHandler = this;
	var probeInterval = setInterval(function () {
	_pluginHandler.searchPlugin();
	if (_pluginHandler.plugin_found) {
	clearInterval(probeInterval);
	_pluginHandler.initializePlugin();
	}
	}, 500);*/
            }
        }
	...
	*/
        ...
        this.connect = function() {
            ...
            if (this.connected == false) {
                ...
                url = url.replace("SERIAL_MONITOR_META", JSON.stringify({
                    "baudrate": speed,
                ...
                if ($("#cb_cf_ports").val() != null && $("#cb_cf_ports").val() != "") {
                    ...
                    this.connected = true;
                    var pl = this;
                    $("#cb_cf_serial_monitor_connect").html("Disconnect").unbind('click').click(function() {
                        pl.disconnect()
                    });
                    $("#serial_hud").html("");
                    ...
                    document.getElementById('plugin0').serialRead(
                        ...
                        function(from, line) {
                            ...
                            url = url.replace("PLUGIN_SERIAL_MONITOR_RETVAL_META", JSON.stringify({
                                "retVal": line,
                                "version": (window.plugin_version === 'undefined' || window.plugin_version === null) ? "undefined" : window.plugin_version,
                                "url": window.location.pathname,
                                "OS": {
        ...
        this.disconnect = function(notified) {
            ...
            if (this.connected == true) {
                if (notified == false) {
                    ...
                    url = url.replace("SERIAL_MONITOR_DISC_META", JSON.stringify({
                ...
                var pl = this;
                $("#cb_cf_serial_monitor_connect").html("<i class='icon-list-alt'></i> Open Serial Monitor").unbind('click').click(function() {
                    pl.connect()
                });
                this.connected = false;
        ...
        this.plugin_error_logger = function(from, msg, status) {
            if (typeof status == 'undefined' || status == 0) {
                ...
                url = url.replace("PLUGIN_ERROR_META", JSON.stringify({
                    "message": msg,
                    "version": (window.plugin_version === 'undefined' || window.plugin_version === null) ? "undefined" : window.plugin_version,
                    "url": window.location.pathname,
                    "tabID": _that.tabID,
                ...
                url = url.replace("PLUGIN_WARNING_META", JSON.stringify({
                    "message": msg,
                    "version": (window.plugin_version === 'undefined' || window.plugin_version === null) ? "undefined" : window.plugin_version,
                    "url": window.location.pathname,
                    "tabID": _that.tabID,
    ...
    if ($("select#cb_cf_boards").length > 0) {
        $("#cb_cf_boards").append($('<option></option>').html("Loading Boards...")).attr('disabled', 'disabled').click(function() {
            cb.clickedBoard()
        }).change(function() {
            cb.saveBoard()
        });
    ...
    if ($("select#cb_cf_ports").length > 0) {
        $("#cb_cf_ports").click(function() {
            cb.pluginHandler.clickedPort()
        }).change(function() {
            cb.pluginHandler.savePort()
    ...
    if ($("button#cb_cf_flash_with_prog_btn").length > 0) {
        $("#cb_cf_flash_with_prog_btn").click(function() {
            cb.usbflashWithProgrammer()
        }).attr('disabled', 'disabled');
        this.loaded_elements.push("cb_cf_flash_with_prog_btn");
    }
    if ($("select#cb_cf_baud_rates").length > 0) {
        $("#cb_cf_baud_rates").append("<option>9600</option>" + "<option>19200</option>" + "<option>28800</option>" + "<option>38400</option>" + "<option>57600</option>" + "<option>115200</option>").attr('disabled', 'disabled');
    ...
    this.saveBoard = function() {
        ...
        if (typeof Lawnchair !== 'undefined') {
            new Lawnchair(function() {
                this.save({
                    key: 'board',
                    name: $("#cb_cf_boards option:selected").text()
                });
            });
        }
        ...
        url = url.replace("SAVEBOARDMETA", JSON.stringify({
            "oldBoard": oldBoard,
            "newBoard": newBoard,
    ...
    this.saveProgrammer = function() {
        if (typeof Lawnchair !== 'undefined') {
            new Lawnchair(function() {
                this.save({
                    key: 'programmer',
                    name: $("#cb_cf_programmers option:selected").text()
                });
            });
        }
        ...
        url = url.replace("SAVEPROGRAMMERMETA", JSON.stringify({
    ...
    this.clickedBoard = function() {
        ...
        url = url.replace("CLICK_BOARD_META", JSON.stringify({
            "selectedBoard": board,
            "tabID": this.pluginHandler.tabID
        }));
        $.get(url);
    ...
    this.clickedProgrammer = function() {
        ...
        url = url.replace("CLICK_PROGRAMMER_META", JSON.stringify({
    ...
    this.usbflash = function() {
        ...
        url = url.replace("RUN_BUTTON_META", JSON.stringify({
            "port": $("#cb_cf_ports").val(),
    ...
    this.usbflashWithProgrammer = function() {
        ...
        url = url.replace("RUN_WITH_PROG_BUTTON_META", JSON.stringify({
            "port": $("#cb_cf_ports").val(),
            "board": $("#cb_cf_boards option:selected").text(),
            "programmer": $("#cb_cf_programmers option:selected").text(),
            "tabID": this.pluginHandler.tabID
    ...
    this.verify = function() {
        ...
        url = url.replace("VERIFY_META", JSON.stringify({
            "selectedBoard": board,
            "tabID": this.pluginHandler.tabID
        }));
        $.get(url);
    ...
    this.burn_bootloader = function() {
        ...
        if (this.pluginHandler.canBurnBootloader(this.selectedProgrammer)) {
            ...
            url = url.replace("UPLOAD_BOOTLOADER_META", JSON.stringify({
            ...
            if (typeof this.selectedBoard['bootloader']['file'] === "undefined") {
                ...
                var cb = this;
                $.get("\x2Fbootloader\x2F" + this.selectedBoard['bootloader']['file'].replace(".hex", ".txt")).success(function(data) {
